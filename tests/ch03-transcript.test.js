...
describe('Message 工厂方法', () => {
  ...
  it('fromAssistantResponse — tool_call + reasoning', () => {
    const resp = new ProviderResponse({
      ...
      tool_args: { expr: '2+2' },
      tool_call_id: 'call-1',
      reasoning_text: 'I will calculate',
      reasoning_metadata: { signature: 'sig-abc' },
    });
    const m = Message.fromAssistantResponse(resp);
    expect(m.blocks).toHaveLength(2);
    expect(m.blocks[0].kind).toBe('reasoning');
...
describe('第 3 章 loop — try/except 修复 Break 1 & 3', () => {
  it('Break 1 fixed: unknown tool → is_error 返回模型（不崩）', () => {
    ...
    const result = run(mock, {}, [], 'What is 2+2?');
    expect(result).toBe('I tried but failed');
  });

  it('Break 3 fixed: tool throws → is_error 返回模型（不崩）', () => {
    ...
    expect(result).toBe('I recovered');
  });

  it('Happy path: calculator（和 Ch 2 一致）', () => {
    ...
    const result = run(mock, { calc }, schemas, 'What is 2 + 2?');
  ...
  it('Break 4: 超过 MAX_ITERATIONS 仍抛出', () => {
    ...
    expect(() => run(mock, { echo }, [], 'loop')).toThrow(/did not finish/);
  });

  it('system prompt 传递', () => {
    const mock = new MockProvider([
      new ProviderResponse({ text: 'You are helpful!' }),
    ]);

    const result = run(mock, {}, [], 'Hi', 'Be helpful');
...
describe('Adapter match 穷举', () => {
  // 测试 _blockToAnthropic — 通过 AnthropicProvider 间接测
  it('所有 4 种 block 都能翻译为 Anthropic 格式', async () => {
    // 不真调 API，只验证消息的 content 构建
    const msg = new Message('assistant', [
      ReasoningBlock('thinking...', { signature: 'sig-abc' }),
      ToolCall('call-1', 'calc', { expression: '2+2' }),
    ]);

    // 只需要验证 block 都能遍历（不抛 unknown kind）
